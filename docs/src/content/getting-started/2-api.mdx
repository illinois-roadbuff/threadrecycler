...
### `recycler.wrap(UseSignal: boolean, callback: function, ...: any)`
Creates and runs a thread using `coroutine.create()` and `coroutine.resume()`. If `UseSignal` is true, the function must have an index.
```lua copy filename="some-script.lua"
