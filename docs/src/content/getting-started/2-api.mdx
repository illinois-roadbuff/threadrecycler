...
local ThreadPool = ThreadRecycler.erect({
    ["InitialThreadCount"] = 60, -- Default initial thread count 
	["CachedLifetime"] = 60,  -- Default cached lifetime 
	["EnableStatRecording"] = true, -- Enables default recording of stats 
	["Logger"] = warn, -- Default log method; doesn't do anything right now. Coming in the future... 
	["Debug"] = false -- Default debug configuration. Doesn't do anything right now. Coming in the future... 
})
```

### `ThreadPool.autocompatible()`
`ThreadPool.autocompatible(ThreadPool, ManualRecycle: boolean, callback: function, ...: any) -> boolean, number | string)`
